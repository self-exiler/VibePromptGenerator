...
class DataStore {
    ...
    async init() {
        ...
        try {
            ...
            // Initialize localStorage with defaults if empty or invalid
            const storedLangs = localStorage.getItem(STORAGE_KEYS.LANGS);
            const langs = storedLangs ? JSON.parse(storedLangs) : [];
            
            if (!storedLangs || !Array.isArray(langs) || langs.length === 0) {
                console.log('Languages localStorage is empty or invalid, loading from defaultData');
